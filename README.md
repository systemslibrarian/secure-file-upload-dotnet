...
- **`SixLabors.ImageSharp` pinned to `[3.1.12,4.0.0)`.** ImageSharp moved to a commercial Six Labors license at v4 — the build fails without a paid license key, and shipping it would push that obligation onto every consumer. The range still admits 3.1.x security patches, which matters because ImageSharp is fed attacker-controlled bytes by design.
...
```json
{
  "FileUpload": {
    ...
    "MaxNonJpegDecodePixels": 40000000,
...
| `FileUpload:MaxReencodeDecodePixels` / `MaxNonJpegDecodePixels` | Decode-memory guards (defaults `50000000` / `40000000`, raised in 3.3.1). Read from headers only, so an over-cap image is refused before any pixels are materialized. JPEG gets the higher ceiling because it has a reduced-resolution decode path; PNG and WebP do not and materialize a full bitmap. 40 MP clears a 600 dpi A4 scan (~35 MP) and bounds one decode at ~160 MB. What happens over the cap follows `FileUpload:RejectOnRecompressFailure`: refuse the upload (default) or store the original undecoded. No decode occurs either way, so the memory bound holds in both. |
